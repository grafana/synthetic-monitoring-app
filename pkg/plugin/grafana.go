...
// devAppURL is Grafana's own address when GrafanaConfig.AppURL() is unavailable.
// Inside the dev container this is Grafana itself.
...
//
// Requests carry the plugin's own service account token (see pluginToken), which
// authenticates the plugin to Grafana. It says nothing about who is asking for
// the data -- that is established separately, by the authorizer, before query is
// ever called.
