...
import { Box, Button, Checkbox, Combobox, Field, IconButton, Input, Label, Stack, Switch, useStyles2 } from '@grafana/ui';
        ...
        {Boolean(fields.length) && (
          <Stack direction={`column`}>
            ...
            {fields.map((field, index) => {
              ...
              const fieldNameId = `Validation Field Name ${userIndex}`;
              
              return (
                <div className={styles.validationGrid} key={field.id}>
                  <Field  data-fs-element={`Regex validation field name ${index}`} label={fieldNameId} htmlFor={fieldNameId}>
                    <Controller
                      render={({ field }) => {
