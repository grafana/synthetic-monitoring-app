...
export const FormLayout2 = ({ children }: FormLayoutProps) => {
  const { handleSubmit: formSubmit } = useFormContext<CheckFormValues>();
  const styles = useStyles2(getStyles);
  const { activeSection, setActiveSection, goToSection, setVisited } = useFormLayoutContext();
  ...
  return (
    <form onSubmit={formSubmit(handleSubmit, handleError)} className={styles.wrapper}>
      <div className={styles.container}>
        <FormSidebar2 sections={formSections} />
        <div
          className={css({
            display: 'flex',
...
FormLayout2.Section = FormSection;
