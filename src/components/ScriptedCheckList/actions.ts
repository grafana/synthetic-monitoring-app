...
export const enableSelectedChecks = async (instance: GrafanaInstances, selectedChecks: Check[]) => {
  const checkUpdates = selectedChecks.map((check) => {
    if (!check) {
  ...
  const notUpdatedCount = selectedChecks.length - resolvedCheckUpdates.length;
...
export const disableSelectedChecks = async (instance: GrafanaInstances, selectedChecks: Check[]) => {
  const checkUpdates = selectedChecks.map((check) => {
    if (!check) {
      return Promise.reject('Could not find check with specified id');
    }
...
export const deleteSelectedChecks = async (instance: GrafanaInstances, selectedChecks: Check[]) => {
  const checkDeletions = selectedChecks.map((check) => {
    if (check.id) {
      return instance.api?.deleteCheck(check.id);
    }
    return Promise.reject('Could not find check with specified id');
  });
