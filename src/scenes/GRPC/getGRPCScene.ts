...
export function getGRPCScene({ metrics, logs }: DashboardSceneAppConfig, checks: Check[], newUptimeQuery = false) {
  return () => {
    if (checks.length === 0) {
    ...
    const { job, instance, probe } = getVariables(CheckType.GRPC, metrics, checks);
