...
/** Label matches first; query-string fallback for manually-created SLOs; deduped by object identity. */
export function getMatchingSlosForSmCheck(slos: Slo[], checkId: string, job: string): Slo[] {
  const byLabel = filterSlosByLabel(slos, checkId);
  const byLabelSet = new Set(byLabel);
  const byQuery = slos.filter((slo) => !byLabelSet.has(slo) && sloMatchesSmCheck(slo, job));
