    ...
    queries: [
      {
        ...
        ceil(
          ...
          sum by (instance, job) (increase(probe_all_success_sum{instance="$instance", job="$job"}[${minStep}]))
          /
          # the total number of times we checked across all probes
          (sum by (instance, job) (increase(probe_all_success_count{instance="$instance", job="$job"}[${minStep}])) + 1) # + 1 because we want to make sure it goes to 1, not 2
        )`,
        hide: false,
        instant: false,
        interval: minStep,
