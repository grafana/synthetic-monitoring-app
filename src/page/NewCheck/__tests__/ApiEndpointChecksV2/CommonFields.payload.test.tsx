...
const checkTypeList = Object.keys(FALLBACK_CHECK_MAP) as CheckType[];

describe('(new) Api endpoint checks - common fields payload', () => {
  checkTypeList.forEach((checkType) => {
    describe(`${checkType}`, () => {
      it(`can submit the form with the minimum required fields`, async () => {
        const { user, read } = await renderNewForm(checkType, NewCheckV2);
