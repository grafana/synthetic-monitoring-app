...
cat << EOF > config.json
{
  ...
  "pull_request_message": "Triggered by Synthetic Monitoring App GitHub Actions. NOTE: staging does not refer directly to an environment it refers to stacks associated with the dev 'wave'. See [here](https://github.com/grafana/deployment_tools/blob/master/ksonnet/environments/hosted-grafana/waves/provisioned-plugins/README.md#waves) for more info.",
